...
const AddProductForm = () => {
  ...
  const inputItems = [
    ...
    { id: "inventory", label: "موجودی", type: "number" },
    { id: "barcode", label: "کد محصول", type: "number" },
    { id: "numberInPack", label: "تعداد در بسته", type: "number" },
    { id: "coloring", label: "تعداد رنگ بندی", type: "number" },
    { id: "size", label: "ابعاد", type: "text" },
    { id: "image", label: "تصویر", type: "file" },
  ];
      ...
      <Form className="rtl max-w-lg mx-auto mt-8 p-6 bg-white rounded shadow-lg">
        <div className="flex justify-between flex-wrap">
          {inputItems.map((item, index) => (
            <div
              className={`mb-4 ${index === 0 ? "w-full" : " w-1/2"} px-2 `}
              key={index}
            >
              <label
                htmlFor={item.id}
